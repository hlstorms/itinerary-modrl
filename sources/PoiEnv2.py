...
class poi_env2:
    ...
    def reset(self, initial_poi, total_time, date, user_id):
        # reset environment when new user is introduced
    ...
    def step(self, poi_dest):
        ...
        preference_match = sum([(self.preferences[x])*tags_binary[x] for x in range(0,5)])
        ##### Reward ####
        reward_st = popularity/(poi_popularity+0.1) + (time_visit) / (time_crowd + time_distance + time_visit) * time_visit / 5
        reward_us = sum([(self.preferences[x])*tags_binary[x] for x in range(0,5)])
        
        reward = np.array([reward_st, reward_us])
        ...
        if len(self.action_space) == 0:
            done_temp = True # ---> done = True when there are not POIs left to visit in the remaining time
    ...
    def crowding_wait(self, poi_dest):
        ...
        self.df_crowding.loc[(self.df_crowding['poi'] == poi_dest) & (self.df_crowding['holiday'] == get_holiday(self.poi_date)) & (self.df_crowding['hour'] == crowd_range.hour)][
            "val_stim"].values
        # if there is no data, I assume a crowd of no queue --> #TODO: make this mean of holiday / hour (not just no queue)
