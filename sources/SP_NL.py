...
def initialization_sp_nl(layer_size, input_layer, output_layer):
...
def train_model_sp_nl(model, memory, batch_size, poi_set_len, gamma=0.96):
...
def SP_NL(environment, neural_network, trials, batch_size, time_input, poi_start, date_input, group_id,
    ...
    for trial in range(trials):
        ...
        while done == False:  # check if all actions are done
            ...
            train_model_sp_nl(neural_network, experience_buffer, batch_size, len(environment.poi_set))
            s = s_1
            score += r_scalarized
            matches += match
